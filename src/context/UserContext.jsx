  ...
  useEffect(() => {
    const fetchInitialProfile = async () => {
      ...
      if (!token) {
        ...
        setUserProfile(prev => ({ ...prev, isAuthenticated: false })); // Set authenticated to false
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (response.status === 401 || response.status === 403 || response.status === 404) {
            ...
            setUserProfile(prev => ({ ...prev, isAuthenticated: false })); // Mark as unauthenticated
            ...
            setUserProfile(prev => ({ ...prev, isAuthenticated: false })); // Assume unauthenticated on other errors too
          }
        }
      } catch (error) {
        console.error("Error fetching initial user profile for context:", error);
        setUserProfile(prev => ({ ...prev, isAuthenticated: false })); // Assume unauthenticated on fetch error
      } finally {
        setLoadingUserContext(false);
      }
    };
