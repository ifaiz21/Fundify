...
const Chatbot = ({ onClose }) => {
  ...
  const backendUrl =
    process.env.REACT_APP_BACKEND_URL || "http://localhost:5000"; // Fallback for local dev
  ...
  const handleSendMessage = async (messageText) => {
    ...
    try {
      ...
      console.log(
        "Fetch response received. Status OK:",
        response.ok,
        "Status:",
        response.status
      );
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorDetails = `HTTP error! status: ${
            response.status
          }. Response text: ${textError.substring(0, 100)}...`;
        }
  ...
  return (
    <div className="fixed right-4 sm:right-8 top-1/2 -translate-y-1/2 z-50 flex justify-center items-center font-inter">
      {/* Slightly reduced width and height for iPhone 12 Pro */}
      <div className="relative w-[85vw] sm:w-[350px] max-w-[340px] h-[75vh] sm:h-[550px] max-h-[550px] bg-[#222B45] rounded-2xl sm:rounded-3xl shadow-xl flex flex-col overflow-hidden border border-white/10">
        {/* Chatbot Header - Dark background, white text with reduced gap */}
        <div className="flex flex-col p-1.5 sm:p-3 bg-[#111827] border-b border-[#313B5B]">
          <div className="flex items-center justify-between space-x-0.5 sm:space-x-1">
            <div className="flex items-center space-x-1 sm:space-x-2">
              {/* Fundify Logo in a circle with active indicator */}
              <div className="relative w-8 sm:w-10 h-8 sm:h-10 rounded-full bg-gray-700 flex items-center justify-center overflow-hidden flex-shrink-0">
                <img
                  src="/Images/Chatbot-Logo.png"
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src =
                      "https://placehold.co/32x32/transparent/white?text=LOGO_ERR";
                  }}
                  alt="Fundify Logo"
                  className="w-full h-full object-cover rounded-full"
                />
                {/* Green active dot - Adjusted border to match header background */}
                <div className="absolute bottom-0 right-0 w-2 sm:w-3 h-2 sm:h-3 bg-green-500 rounded-full border-2 border-[#111827]"></div>
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-2 sm:space-y-4 bg-[#1f2937] scrollbar-hide">
          ...
          {messages.map((message, index) => (
            ...
            >
              <div
                className={`max-w-[70%] p-2 sm:p-3 rounded-xl shadow-sm ${
                  message.sender === "user"
          ...
          {isTyping && (
            <div className="flex justify-start">
              <div className="max-w-[70%] p-2 sm:p-3 rounded-xl shadow-sm bg-[#4B5842] text-white rounded-bl-none">
                Typing...
              </div>
            </div>
          )}
        ...
        <div className="p-2 sm:p-4 bg-[#111827] border-t border-white/10 flex items-center space-x-1 sm:space-x-2 relative">
          {/* Input field wrapper with smiley icon */}
          <div className="flex-1 flex items-center bg-[#2C3759] text-white rounded-full border border-[#3D527B] px-2 sm:px-3 py-1 sm:py-2">
            <button
              onClick={() => setShowEmojiPicker(!showEmojiPicker)}
              className="flex-shrink-0 p-0.5 sm:p-1 -ml-0.5 sm:-ml-1 focus:outline-none"
            >
              <img
                ...
                onError={(e) => {
                  e.target.onerror = null;
                  e.target.src =
                    "https://placehold.co/20x20/transparent/white?text=😊";
                }}
                alt="Smiley Icon"
                className="w-4 sm:w-5 h-4 sm:h-5 object-contain cursor-pointer"
              />
            </button>
            <input
              type="text"
              className="flex-1 bg-transparent text-white placeholder-gray-400 focus:outline-none pl-1 sm:pl-2 pr-0.5 sm:pr-1 text-xs sm:text-sm"
          ...
          <button
            ...
            className="bg-[#3366FF] hover:bg-[#255EEF] text-white p-1.5 sm:p-3 rounded-full flex-shrink-0 flex items-center justify-center shadow-lg transition-colors duration-200"
          >
            <svg
              className="w-4 sm:w-6 h-4 sm:h-6 transform rotate-90"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
          ...
          {/* Emoji Picker */}
          {showEmojiPicker && (
            <div className="absolute bottom-full left-0 mb-1 sm:mb-2 p-1 sm:p-2 bg-[#2C3759] rounded-lg shadow-lg grid grid-cols-5 gap-0.5 sm:gap-1 z-10">
              {emojis.map((emoji, index) => (
                <span
                  key={index}
                  className="cursor-pointer text-lg sm:text-2xl hover:bg-[#3D527B] rounded p-0.5 sm:p-1"
