...
import { Search, ChevronLeft, ChevronRight, ChevronDown, Eye, X, Check } from "lucide-react" // Added Eye, X, Check icons
import axios from "axios"
import { useNavigate } from "react-router-dom"
import { showSuccessMessage, showErrorMessage } from '../../utils/toast'; // Import toast notifications

const VerificationPage = () => {
  const navigate = useNavigate();

  // State for active tab (Campaign Verification or KYC Verification)
  const [activeTab, setActiveTab] = useState("Campaign Verification");

  // State for Campaign Verifications
  ...
  // State for KYC Verifications
  const [kycApplications, setKycApplications] = useState([]);
  const [selectedKYCApplication, setSelectedKYCApplication] = useState(null); // For modal
  const [isKYCModalOpen, setIsKYCModalOpen] = useState(false); // For modal visibility
  ...
  // Function to fetch campaign verifications and stats
  const fetchCampaignVerifications = useCallback(async () => {
    ...
    try {
      ...
      // Assuming backend provides overall stats for campaigns
  ...
  // Function to fetch KYC applications and stats
  const fetchKYCApplications = useCallback(async () => {
    ...
    try {
      ...
      // MODIFICATION: Corrected the API endpoint for KYC applications
      ...
      const fetchedStats = response.data?.stats || { // Assuming backend provides these stats
  ...
  }, [activeTab, fetchCampaignVerifications, fetchKYCApplications]); // Dependencies include activeTab and the memoized fetch functions


  // Handlers for Campaign Approve/Reject actions
  const handleApproveRejectCampaign = async (campaignId, status) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        fetchCampaignVerifications(); // Refresh the list immediately after action
      } else {
  ...
  const handleViewCampaign = (campaignId) => {
    navigate(`/project/${campaignId}`); // Corrected route to match App.jsx
  };

  // Handlers for KYC Approve/Reject actions
  const handleApproveRejectKYC = async (userId, status) => {
    if (!window.confirm(`Are you sure you want to ${status} this KYC application?`)) {
      return;
    }
    try {
      const token = localStorage.getItem('token');
      ...
      // MODIFICATION: Corrected the API endpoint for KYC approval/rejection
      ...
      if (response.status === 200) {
        ...
        fetchKYCApplications(); // Refresh the list immediately after action
        setIsKYCModalOpen(false); // Close modal after action
      } else {
  ...
  // Handle viewing KYC details in a modal
  ...
  // Filtering logic for campaigns and KYC applications
  const getFilteredItems = (items, type) => {
    return items.filter((item) => {
      // For KYC, use item.userId if fullName isn't always present and unique enough for search
      const nameField = type === 'campaign' ? (item.creator?.name || '') : (item.fullName || item.userId || '');
      // For KYC, use an identifiable field like email or a unique ID if kycId isn't available
      const titleField = type === 'campaign' ? item.title : (item.email || item.userId || '');
      const descriptionField = type === 'campaign' ? item.description : ''; // KYC might not have a description field in table
      ...
      const itemDate = new Date(item.createdAt || item.submissionDate); // Use submissionDate for KYC if available
                      ...
                      {displayedItems.length > 0 ? (
                        displayedItems.map((kyc) => (
                          <tr key={kyc._id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-6 py-4 text-sm font-medium text-gray-900">{kyc.fullName || kyc.userId || 'N/A'}</td>
                            ...
                            <td className="px-2 py-0.5">
                              {kyc.status === "Pending" || kyc.status === "Pending Review" ? (
                                <div className="flex space-x-2 justify-center">
                                  ...
                                  <button
                                    onClick={() => handleApproveRejectKYC(kyc.userId, "approve")}
                                  ...
                                  <button
                                    onClick={() => handleApproveRejectKYC(kyc.userId, "reject")}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end p-5 border-t border-gray-200 space-x-3">
              <button
                onClick={() => handleApproveRejectKYC(selectedKYCApplication.userId, "approve")}
              ...
              <button
                onClick={() => handleApproveRejectKYC(selectedKYCApplication.userId, "reject")}
