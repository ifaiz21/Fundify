...
const ContactUsPage = () => {
    // 2. Redux store se user ka token (aur doosra data) hasil karein
    const { currentUser } = useSelector((state) => state.user);

    const [showSubmitConfirmation, setShowSubmitConfirmation] = useState(false)
    const [formData, setFormData] = useState({
        // Agar user logged in hai to uska naam aur email pehle se bhar dein
    ...
    const handleConfirmSubmit = async () => {
        ...
        try {
            // 3. Headers ko dynamically banayein
            const headers = {
                'Content-Type': 'application/json',
            };

            // Agar user logged in hai (token mojood hai), to Authorization header add karein
            ...
            const response = await fetch('https://server-fundify.up.railway.app/api/contactus', {
                method: 'POST',
                headers: headers, // Yahan updated headers istemal karein
                body: JSON.stringify(formData),
            });

            if (response.ok) {
                showSuccessMessage("Your support request has been submitted successfully! We'll get back to you soon. ✅");
                // Form reset karte waqt bhi user ka data pehle se bhar dein agar woh logged in hai
                setFormData({
                    name: currentUser?.name || "",
                    email: currentUser?.email || "",
                    issue: "",
                            ...
                            <div className="bg-white p-6 rounded-lg shadow-lg">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center space-x-4">
                                        <div className="bg-[#E6F0E4] rounded-full p-3"><EmailIcon /></div>
                                        <div>
                                    ...
                                    <div className="flex items-center space-x-4">
                                        <div className="bg-[#E6F0E4] rounded-full p-3"><PhoneIcon /></div>
                                        <div>
                            ...
                            <form onSubmit={handleSubmitRequest} className="space-y-5">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <input type="text" id="name" value={formData.name} onChange={handleInputChange} placeholder="John Doe" className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4B5842] focus:border-transparent transition" required />
                                    </div>
                                    <div>
                                        <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1">E-mail<span className="text-red-500">*</span></label>
                                        <input type="email" id="email" value={formData.email} onChange={handleInputChange} placeholder="you@example.com" className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4B5842] focus:border-transparent transition" required />
                                    </div>
                                </div>
                                <div>
                                    <label htmlFor="issue" className="block text-sm font-medium text-gray-700 mb-1">Select Your Issue<span className="text-red-500">*</span></label>
                                    <select id="issue" value={formData.issue} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4B5842] focus:border-transparent bg-white transition" required>
                                ...
                                <div>
                                    ...
                                    <input type="text" id="subject" value={formData.subject} onChange={handleInputChange} placeholder="e.g., Issue with my recent contribution" maxLength={96} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4B5842] focus:border-transparent transition" required />
                                ...
                                <div>
                                    ...
                                    <textarea id="message" value={formData.message} onChange={handleInputChange} placeholder="Please describe your issue in detail here..." rows="4" className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4B5842] focus:border-transparent transition" required></textarea>
                                </div>
                                <div className="text-center pt-2">
                                    <button type="submit" className="w-full sm:w-auto bg-[#4B5842] text-white px-8 py-3 rounded-md hover:bg-[#3A4433] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#4B5842] font-semibold transition-transform transform hover:scale-105">Submit Request</button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-lg shadow-xl max-w-sm w-full mx-auto">
                        <div className="p-6 text-center">
                            ...
                            <p className="text-gray-600 mb-6">Please review your information. Are you sure everything is correct?</p>
                            <div className="flex justify-center gap-4">
                                <button onClick={handleCancelSubmit} className="w-full px-4 py-2 text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 transition-colors font-semibold">Cancel</button>
                                <button onClick={handleConfirmSubmit} className="w-full px-4 py-2 text-white bg-[#4B5842] rounded-md hover:bg-[#3A4433] transition-colors font-semibold">Confirm</button>
                            </div>
                        </div>
                    </div>
                </div>
