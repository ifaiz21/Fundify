...
function CampaignEditor() {
  ...
  const applyFormat = (format) => {
    if (textareaRef.current) {
      ...
      const selectedText = campaignData.content.substring(selectionStart, selectionEnd)
      let newContent = campaignData.content

      switch (format) {
        case 'bold':
          newContent = campaignData.content.substring(0, selectionStart) + `**${selectedText}**` + campaignData.content.substring(selectionEnd)
          break
        case 'italic':
          newContent = campaignData.content.substring(0, selectionStart) + `*${selectedText}*` + campaignData.content.substring(selectionEnd)
          break
        case 'underline':
          newContent = campaignData.content.substring(0, selectionStart) + `<u>${selectedText}</u>` + campaignData.content.substring(selectionEnd)
          break
        case 'align-left':
          // You might need to use CSS classes for more complex formatting
          break
        case 'align-center':
          break
        case 'align-right':
        ...
        case 'unordered-list':
          ...
          newContent = campaignData.content.substring(0, selectionStart) + lines.join('\n') + campaignData.content.substring(selectionEnd);
          break
        case 'ordered-list':
          const numberedLines = selectedText.split('\n').map((line, index) => line ? `${index + 1}. ${line}` : '');
          newContent = campaignData.content.substring(0, selectionStart) + numberedLines.join('\n') + campaignData.content.substring(selectionEnd);
          break
        default:
          break
      }

      setCampaignData(prevData => ({ ...prevData, content: newContent }));
    }
  }
            ...
            <div className="editor-container">
              <div className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div className="toolbar px-2 py-2 border-b border-gray-200 flex flex-wrap">
                  <button
                    className="p-1 mx-1 rounded hover:bg-gray-100"
                  ...
                  {/* You'll need to implement functionality for the table and image buttons */}
                  {/* <button className="p-1 mx-1 rounded hover:bg-gray-100">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="3" y1="9" x2="21" y2="9"></line><line x1="3" y1="15" x2="21" y2="15"></line><line x1="9" y1="21" x2="9" y2="3"></line></svg>
                  </button>
                  <button className="p-1 mx-1 rounded hover:bg-gray-100">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
                  </button> */}
                </div>
